...
async function onSubmit() {
  ...
  auth_store.login(data).then(
    (resp)=>{
      ...
      if(resp.status && auth_store.role==='user'){
        router.push({path: '/userdashboard'})
      }
      else if(resp.status && auth_store.role==='admin'){
        router.push({path: '/librariandashboard'})
      }
      else{
    ...
    <div class="d-flex justify-content-center">
      <form class="w-50 justify-content-center" @submit.prevent = "onSubmit">
        ...
        <div class="d-flex justify-content-center mt-2">
          ...
          <RouterLink class="btn btn-primary justify-content-center" to="/register" >Register</RouterLink>
        </div>
      </form>
    </div>
  </div>
